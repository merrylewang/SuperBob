...
public class PlatformView extends SurfaceView implements Runnable {
    ...
    //some of the brackets may be wrong, look to fix
    private void update() {
    ...
    private void drawBackground(int start, int stop) {
        ...
        for (Background bg : lm.backgrounds) {
            if (bg.z < start && bg.z > stop) {
                ...
                //Clip everything off-screen
                if (!vp.clipObjects(-1, bg.y, 1000, bg.height)) {
                    float floatstartY = ((vp.getyCenter() - ((vp.getViewportWorldCenterY() - bg.y)
                            * vp.getPixelsPerMeterY())));
                    int startY = (int) floatstartY;

                    float floatendY = ((vp.getyCenter() -
                            ((vp.getViewportWorldCenterY() - bg.endY) *
                                    vp.getPixelsPerMeterY())));
